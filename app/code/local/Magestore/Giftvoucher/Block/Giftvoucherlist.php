...
class Magestore_Giftvoucher_Block_Giftvoucherlist extends Mage_Core_Block_Template {
    ...
    protected function _construct() {
        ...
        $collection->getSelect()
            ->joinleft(array('voucher_table' => $voucherTable), 'main_table.voucher_id = voucher_table.giftvoucher_id', array('recipient_name', 'gift_code', 'balance', 'currency', 'status', 'customer_check_id' => 'voucher_table.customer_id', 'recipient_email','customer_email'))
    ...
    public function getActions($row) {
        ...
        if ($row->getStatus() < Magestore_Giftvoucher_Model_Status::STATUS_DISABLED) {
            $action .= ' | <a href="' . $this->getUrl('*/*/print', array('id' => $row->getId())) . '" >' . $this->__('Print') . '</a>';
