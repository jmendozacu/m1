<?php
class Infomodus_Upstablerates_Model_Mysql4_Carrier_Upstablerates extends Mage_Core_Model_Mysql4_Abstract
{
    protected function _construct()
    {
        $this->_init('shipping/upstablerates', 'pk');
    }
    ...
    public function loadPk($id)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTableName('upstablerates_shipping/upstablerates');
    ...
    {
        $csvFile = $_FILES["groups"]["tmp_name"]["upstablerates"]["fields"]["import"]["value"];
        ...
        if (!empty($csvFile)) {
            ...
            if (isset($_POST['groups']['upstablerates']['fields']['condition_name']['inherit'])) {
                $conditionName = (string)Mage::getConfig()->getNode('default/carriers/upstablerates/condition_name');
            }
            else
            {
                $conditionName = $_POST['groups']['upstablerates']['fields']['condition_name']['value'];
            }

            $conditionFullName = Mage::getModel('upstablerates_shipping/carrier_upstablerates')->getCode('condition_name_short', $conditionName);
    ...
    {
        if (Mage::app()->getConfig('carriers/upstablerates/separator') == 1) {
            $separator = ';';
        }
