...
class MageWorx_OrdersGrid_Model_Order_Invoice extends Mage_Core_Model_Abstract
{
    ...
    protected function create(Mage_Sales_Model_Order $order)
    {
        ...
        $savedQtys = array();
        $invoiceCaptureCase = $helper->getDefaultInvoiceCaptureCase();
        if (!$invoiceCaptureCase) {
            $invoiceCaptureCase = Mage_Sales_Model_Order_Invoice::CAPTURE_ONLINE;
        }
        ...
        $invoice->setRequestedCaptureCase($invoiceCaptureCase);
        $invoice->register();
