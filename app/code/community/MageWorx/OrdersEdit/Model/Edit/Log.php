...
class MageWorx_OrdersEdit_Model_Edit_Log extends Mage_Core_Model_Abstract
{
                ...
                switch ($code) {
                    case 'customer_id' :
                        ...
                        if ($fromCustomer->getId()) {
                            $from = $fromCustomer->getFirstname() .
                                ' ' . $fromCustomer->getLastname() . ' (ID: ' . $fromCustomer->getId() . ')';
                        } else {
                            $from = $source->getOrigData('customer_firstname') .
                                ' ' . $source->getOrigData('customer_lastname') . ' (Guest)';
                        }

                        $toCustomer = Mage::getModel('customer/customer')->load($source->getData($code));
                        $to = $toCustomer->getFirstname() .
                            ' ' . $toCustomer->getLastname() . ' (ID: ' . $toCustomer->getId() . ')';
    ...
    public function commitOrderChanges(Mage_Sales_Model_Order $order)
    {
        ...
        foreach ($changes as $code => $diff) {
            ...
            $text .= $labelPrefix;
            if ($code == 'coupon_code' && empty($diff['from'])) {
                $text .= $helper->__("Coupon code \"%s\" was successfully applied.", $diff['to']);
            } else {
        ...
        if (count($this->_itemsChanges)) {
            foreach ($this->_itemsChanges as $item) {
                ...
                if (empty($item['qty_after'])) {
                    ...
                    $text .= $helper->__(
                        "\"%s\" has been added to the order (Qty: %s)",
                            $item['name'],
                            $item['qty_after']
                        ) . $this->eol();
                    ...
                    $text .= $helper->__(
                        "%s item(s) of \"%s\" have been added to the order",
                            $qtyDiff,
                            $item['name']
                        ) . $this->eol();
                    ...
                    $text .= $helper->__(
                        "%s item(s) of \"%s\" have been removed from the order",
                            $qtyDiff,
                            $item['name']
                        ) . $this->eol();
                }
                ...
                // Price changes
                if (isset($item['price_after']) &&
                    isset($item['price_before']) && $item['price_after'] != $item['price_before']
                ) {
                    $text .= $helper->__(
