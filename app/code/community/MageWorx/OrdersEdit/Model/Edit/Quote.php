...
class MageWorx_OrdersEdit_Model_Edit_Quote extends Mage_Core_Model_Abstract
{
    ...
    public function applyDataToQuote(Mage_Sales_Model_Quote $quote, array $data)
    {
        ...
        Mage::dispatchEvent(
            'mwoe_apply_data_to_quote_collect_totals_after', array(
            'quote' => $quote,
            'new_data' => $data,
            )
        );
    ...
    public function addNewItems(Mage_Sales_Model_Quote $quote, $data)
    {
        ...
        if (Mage::registry('ordersedit_order')) {
            Mage::helper('mageworx_ordersedit/edit')->addPendingChanges(
                Mage::registry('ordersedit_order')->getId(), array(
    ...
    protected function validateCouponCode($quote, $couponCode = '')
    {
        ...
        if ($codeLength) {
            if ($isCodeLengthValid && $couponCode == $quote->getCouponCode()) {
                ...
                Mage::helper('mageworx_ordersedit/edit')->addPendingChanges(
                    Mage::registry('ordersedit_order')->getEntityId(),
                    array('coupon_code' => '')
                );
     ...
     */
    public function saveTemporaryItems(
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            $prevFlag = $item->getData('ordersedit_is_temporary');
            $item->setData('ordersedit_is_temporary', $flag)->save();
            if ($prevFlag != $flag && $decreaseStock) {
                $this->removeQtyFromStock($item->getProductId(), $item->getQty());
            }
        }
    }
    ...
    public function clearQuoteItems($item, $checkChild = false)
    {
        ...
        if ($parentItem && $parentItem->getProduct() && $parentItem->getProduct()->getTypeId() != 'configurable') {
            $orderItemAvailableQty = $orderItem->getQtyToCancel() + $orderItem->getQtyToRefund();
            $orderItemQty = $orderItem->getQtyOrdered();
            if ($orderItemAvailableQty < 0.001 &&
                $orderItemQty == $item->getQty() &&
