...
use SquareConnect\Api\CustomersApi as API;
use SquareConnect\Model\RetrieveCustomerResponse as Res;
class Squareup_Omni_Model_Customer_Export_Export extends Mage_Core_Model_Abstract
{
	 ...
	 * «Fix a Square payment bug it Magento 1:
	 * "customer has been merged and cannot be updated or deleted"»
	 * https://www.upwork.com/ab/f/contracts/21419330
	 * @used-by \Squareup_Omni_Model_Observer::customerSaveBefore()
	 * @param C $c
	 */
    function refreshId(C $c) {
		if ($id = $c['squareup_customer_id']) { /** @var string $id */
			$api = new API($this->_apiClient);  /** @var API $api */
			$res = $api->retrieveCustomer($id); /** @var Res $res */
			if ($id !== ($new = $res['customer']['id']) && $new) {
				$c['squareup_customer_id'] = $new;
			}
		}
	}
