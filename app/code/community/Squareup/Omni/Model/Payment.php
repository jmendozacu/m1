...
//        }
    ...
    protected function charge(Varien_Object $payment, $amount, $type='authorize')
    {
        ...
        try {
            ...
            $this->_logHelper->error($e->__toString());
			// 2019-01-13 Dmitry Fedyuk https://www.upwork.com/fl/mage2pro
			// «Format the Square's payment error message on the checkout screen»
			// https://www.upwork.com/ab/f/contracts/21366197
			$r = $e->getResponseBody(); /** @var mixed|stdClass $r */
            Mage::throwException(
				$r instanceof stdClass
				&& isset($r->errors)
				&& is_array($r->errors)
				&& isset($r->errors[0])
				&& isset($r->errors[0]->detail)
				? str_replace(
					'{message}'
					,rtrim($r->errors[0]->detail, '.')
					,Mage::getStoreConfig('payment/squareup_payment/error_message')
				)
            	: $e->getMessage()
			);
        }
